...
Artwork Agent finished / Run now / every day 04:00
  ...
  -> one pbn job at a time: canvasSizes x colorsList (default 40x50, 32x40, 20x25 x 12/24/36/48), HARD, orientation auto
...
- `Print/<stamp>_40x50_24_user.pdf` (one per version): the website's **User PDF** (`template.pdf`): finished painting, painted template with numbers, legend with numbers and colors only;
...
The job is sent with `orientation: auto`: the API compares the artwork's width and height, so a portrait artwork becomes **40x50** and a landscape (or square) one **50x40** (same for 32x40 and 20x25). It crops automatically to the exact ratio without distortion (`cropMode: attention` keeps the most interesting area). The size the API chose is part of the file names.
