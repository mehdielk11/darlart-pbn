...
- `POST /v1/featured`: the featured product image: the artwork (multipart `image` or `imageUrl`) on the blank canvas of the portrait or landscape wall photo, whichever matches its shape (square is portrait). Optional `size` (800-3000, default 1600). Returns `{ template, width, height, contentType: "image/png", image }` with the PNG as base64. Used by the Shopify Uploader (`SHOPIFY-UPLOADER.md`).
- `POST /v1/webp`: a WebP copy of an image (multipart `image` or `imageUrl`), for Shopify product images. Optional `quality` (50-100, default 88) and `maxSide` (64-4096, only shrinks). Returns `{ width, height, bytes, sourceBytes, contentType: "image/webp", image }` with the WebP as base64. Used by the Shopify Uploader.
